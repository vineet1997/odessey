...
12. ~~GitHub Actions cron~~ ✓ 2026-07-20 — `.github/workflows/scrape-district.yml`: runs `npm run scrape:district` every 30 min (+ manual `workflow_dispatch`), commits+pushes `data/showtimes-live.json` only when it actually changed, no secrets required. Vercel's git integration redeploying on that push is the trigger mechanism — no separate deploy hook needed. YAML validated locally (parses correctly); the actual scheduled run on GitHub's infrastructure hasn't been watched yet (no `gh` CLI available in this environment) — **check the Actions tab on the repo to confirm the first scheduled/manual run succeeds**.
13. **Repo is now live**: [github.com/vineet1997/odessey](https://github.com/vineet1997/odessey), pushed as 9 logical commits. Vercel project creation is in progress (Vineet, in parallel).
14. ~~Routing proxy Edge Function~~ ✓ 2026-07-20, **live-verified** — `api/route.ts`, deployed at [odessey-topaz.vercel.app](https://odessey-topaz.vercel.app). Vineet set `GOOGLE_MAPS_SERVER_KEY` in Vercel and redeployed; curl-tested directly against the real endpoint (not just trusted): DRIVE mode returns real Google data (`source: "live"`, DLF Phase 2 → Vasant Vihar area ≈ 40 min / 18.7 km), TRANSIT mode also live (71 min / 22 km — plausibly slower than driving, as expected), malformed input correctly returns 400 `unavailable`, GET instead of POST correctly returns 405. The key, billing, Routes API enablement, and the Edge Function itself are all confirmed working end-to-end on Vercel's real infrastructure — not just typechecked locally. Falls back to a straight-line-distance estimate (conservative 18km/h) if the live call ever fails — this stands in for the not-yet-built precomputed 40×15×3 matrix.
15. ~~Wire real data into the UI~~ ✓ 2026-07-21 — `src/lib/buildRecommendation.ts` + `src/components/Crossing.tsx` + `src/App.tsx`. The four-scene flow now computes a genuinely live recommendation (real venues, real showtimes, real routing) instead of the fixture. Verified locally first via a stubbed-fetch dry run (`scripts/dry-run-recommendation.ts`) against the real data files: for a DLF Phase 2 Gurugram user, the three intents genuinely diverge on real venues (Full Epic → Priya, Worth Every Rupee → a discounted Select City Walk show, Easy Evening → Ambience Gurugram, correctly the closest option). Honest simplifications, all documented in the module's doc comment: no GTFS yet so the "way home" signal is a rough citywide 11:15pm cutoff with generic wording (never a fabricated train/station); journey legs are framed as "CAB" (DRIVE-mode routing), not a real metro line; cab fare is an uncalibrated placeholder formula; the Helm's day (weekday/weekend) answer isn't used yet since the scraper only captures "whatever's showing next," not a queryable future date. Also geocoded and verified real coordinates for all 20 Helm localities.

## Feedback pass (2026-07-21) — logged, not yet all actioned

Vineet checked the live production deploy directly and found:
1. Google Maps directions aren't embedded on the card yet (DESIGN.md calls for the Embed API).
2. Share button doesn't work (was explicitly stubbed in the Result Card build pass, not a regression).
3. "Book on District" button doesn't work (same — stubbed; the real per-venue District URL already exists in `showtimes-live.json` and just needs wiring).
4. No way to go back from the Result screen and reconsider a different locality/day/intent — once you reach the answer, you're stuck there.
5. **Biggest one**: the app should build more trust/value beyond a single recommendation — some way to see the underlying aggregated data (all venues, their cost/quality scores) so the "why" is inspectable, not just asserted. Floated as possibly its own page/view, not just a card tweak.
6. ~~GitHub Actions scraper not running on schedule~~ ✓ RESOLVED — was the diagnosed cold-start, nothing more: the scheduled cron fired on its own (`chore: refresh District showtimes`, commit `fd904ba`) about 2 hours after the workflow was first added, confirming the earlier diagnosis was correct and no fix was needed.
7. Overall UI reads as "too basic" — needs a real design/motion polish pass, not just correctness fixes.

~~3. "Book on District" button~~ ✓ 2026-07-21 — `RecommendationResult` now carries the winning venue's real district.in URL (already scraped, just wasn't threaded through `buildRecommendation.ts`); button opens it in a new tab. Verified for real: each intent's winning venue links to that exact venue's page, and the URL was curl-confirmed to return 200, not just typechecked.

~~4. No way back from Result to reconsider~~ ✓ 2026-07-21 — two controls added to `App.tsx`, deliberately placed OUTSIDE the Result Card (which is a screenshot object per DESIGN.md — navigation chrome doesn't belong on the thing people share): an **intent switcher** (re-run the same locality/day/time-band under a different intent without redoing the Helm — this is DESIGN.md's originally-envisioned "intents switchable in place," now actually built) and a **Start Over** control. Verified in-browser with a stubbed routing response (the real proxy only exists on Vercel, not local dev): confirmed switching from Full Epic (Priya) to Worth Every Rupee (Select City Walk) actually changes the venue while locality/time-band stay fixed, and Start Over correctly returns to the Helm's first screen.

~~1 + 5. Maps embed / trust-aggregate-data page~~ ✓ 2026-07-21, merged into one feature — `src/components/MapExplorer.tsx`. A "See all N venues we checked" toggle on the Result screen reveals a real map of every venue actually scored for the current request (not just the winner), each marker showing real experience score/price/distance on click, color-coded gold/sea/muted for recommendation/runner-up/also-scored. **Deliberate architecture call**: built on Leaflet + free CartoDB dark tiles instead of Google's Maps JavaScript API — our Google key is intentionally IP-restricted and server-only (`api/route.ts`); exposing it to the browser for an interactive map would need a second, separately-restricted key and reopens exactly the client-secret-exposure surface the routing proxy was designed to avoid. Leaflet needs no key at all. `buildRecommendation.ts` now exposes the full scored candidate list (`MapVenue[]`), not just winner+runner-up. Verified in-browser (stubbed routing call locally, same as prior passes): marker count matched the real candidate count exactly, real CartoDB tiles loaded over the network, a clicked marker's popup showed the exact real score/price/distance for that venue, zero console errors.

**Bonus fix (2026-07-21, reported separately from the numbered list above):** the Prologue was being permanently skipped after the very first-ever "Begin" click on a browser — not intermittent, 100% deterministic. Root cause traced (not guessed): the "don't replay the intro for returning users" flag used `localStorage`, which never expires and is shared across *every future visit* from that browser. Fixed by switching to `sessionStorage` in `Prologue.tsx`/`App.tsx` — same intent (a mid-session reload shouldn't replay the intro), but it clears when the tab/browser actually closes, so a genuinely new visit later shows the Prologue again. Verified in-browser through all three cases (fresh state shows it, same-session reload skips it, simulated new session shows it again).

~~2. Share button~~ ✓ 2026-07-21 — PNG export via `html-to-image` (1080px wide, per DESIGN.md), native OS share sheet where available (falls back to download), loading/error states on the button itself. **Real bug found and fixed during verification**: html-to-image's font-embedding step throws on the cross-origin Google Fonts stylesheet (a documented CORS pitfall) — fixed with `skipFonts: true` (harmless, since fonts are already rendering live on the page). **Also discovered**: this project's own browser-verification tooling has a tab that reports as permanently backgrounded (`document.hidden: true`), which suspends `requestAnimationFrame` — and html-to-image's internal image-decode step happens to gate its resolution behind an rAF callback, so the "happy path" hangs specifically in that tool, not for real users with a normal focused tab (this is the same underlying quirk documented earlier for GSAP animations and screenshot capture). Since the tool couldn't confirm the happy path directly, verified the pipeline by parts instead: `toSvg()` resolves correctly against the real card, and manually replicating the remaining steps (Image.onload → canvas.drawImage → toDataURL, bypassing only the rAF gate) produced a valid PNG whose pixel data matched the card's real `--bg` color exactly. Also added a defensive 8s timeout so a real hang (any cause) degrades gracefully instead of leaving the button stuck — directly confirmed the button recovers to normal after the timeout fires.

Remaining: #7 (UI polish pass) — the last one on the original feedback list.
