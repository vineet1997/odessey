...
6. ~~GitHub Actions scraper not running on schedule~~ — investigated via GitHub's public API (not guessing): only 1 run ever exists, and it's the manual `workflow_dispatch` from earlier — the schedule trigger has never fired, despite ~90 minutes elapsed against a 30-min cadence. The workflow itself is valid (proven by that successful manual run). This matches a well-documented GitHub Actions cold-start behavior: brand-new `schedule:` triggers can take 1-3 hours to activate in GitHub's internal scheduler. **Decision: give it time rather than force a fix** — re-check the Actions tab in an hour or two; if still zero scheduled runs by then, that's the real signal something's actually wrong (check Settings → Actions → General isn't restricting scheduled workflows).
...
Remaining, in rough order of what to tackle next: #4 (no way back from Result to reconsider), #2 (Share button), #1 (Maps embed), #5 (trust/aggregate-data page — the big one), #7 (UI polish pass).
