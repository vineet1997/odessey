...
14. ~~Routing proxy Edge Function~~ ✓ 2026-07-20 — `api/route.ts`. Vercel project confirmed live: [vercel.com/vineet1997s-projects/odessey](https://vercel.com/vineet1997s-projects/odessey). Always returns 200 with an explicit `source` field (`"live"` / `"estimated"` / `"unavailable"`) so callers never guess how much to trust a number. Falls back to a straight-line-distance estimate (conservative 18km/h) when the key isn't configured yet or the live call fails — this stands in for the not-yet-built precomputed 40×15×3 matrix, so the app works end-to-end today without waiting on that separate data-build task. Typecheck clean in isolation; `npm run build`/`npm test` unaffected (this file lives outside `src/`, outside the Vite bundle — Vercel builds it separately as an Edge Function). **Not yet verified live** — needs `GOOGLE_MAPS_SERVER_KEY` set in Vercel's dashboard (Settings → Environment Variables) and a deploy, then the actual endpoint should be curl-tested for real before trusting it.
15. Next: GTFS metro pull. Wiring `showtimes-live.json` and the routing proxy into the actual UI (Helm/ResultCard currently still run on the static fixture — this is the next real integration step now that both live-data pieces exist independently). The 4 remaining manual coordinate lookups (Elan Epic Gurugram, Pacific Mall Jasola, PVR Cinemagic Pitampura, Ambience Mall disambiguation) fold into whichever pass touches that data next.
